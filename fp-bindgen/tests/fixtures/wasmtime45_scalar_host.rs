...
{
    linker.func_wrap("kernal-api:v1", "checked", |mut caller: wasmtime::Caller<'_, T>, flag: i32, tiny: i32, total: i64| -> wasmtime::Result<i32> {
    ...
    linker.func_wrap("kernal-api:v1", "reset", |mut caller: wasmtime::Caller<'_, T>| -> wasmtime::Result<()> {
...
pub(crate) fn invoke_guest_measure<T>(store: &mut wasmtime::Store<T>, instance: &wasmtime::Instance, value: i16, ratio: f32) -> wasmtime::Result<u16> {
...
pub(crate) fn invoke_guest_unit<T>(store: &mut wasmtime::Store<T>, instance: &wasmtime::Instance, ) -> wasmtime::Result<()> {
